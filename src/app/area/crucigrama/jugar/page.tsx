...
import { ArrowLeft, CheckCircle, RotateCcw, Loader2, Trophy } from 'lucide-react'
import { setProgresoCrucigrama } from '../page'

// ── Banco de palabras por simulador y nivel ───────────────────────────────────
...
function generarCrucigrama(palabras: { palabra: string; pista: string }[]) {
  ...
  for (let pi = 1; pi < limpias.length; pi++) {
    ...
    // Intentar intersectar con palabras ya colocadas
    ...
    // Si no se pudo intersectar, colocar en espacio libre
    if (!colocada) {
...
function CrucigramaJugarContenido() {
  ...
  // Calcular celdas activas del crucigrama (recortar grid)
  ...
  function getNumero(r: number, c: number): number | undefined {
    return colocadas.find(p =>
      (p.row === r && p.col === c)
    )?.numero
  }

  function esCeldaActiva(r: number, c: number): boolean {
    return grid[r][c] !== null
  }

  function esCeldaSeleccionada(r: number, c: number): boolean {
    if (!seleccionada) return false
    if (seleccionada.direccion === 'H') {
      return r === seleccionada.row && c >= seleccionada.col && c < seleccionada.col + seleccionada.palabra.length
    }
  ...
  function handleInput(valor: string) {
    ...
    // Encontrar siguiente celda vacía en la palabra seleccionada
  ...
  function handleBorrar() {
    if (!seleccionada) return
    // Borrar última letra de la palabra seleccionada
  ...
  function verificarTodo() {
    ...
    colocadas.forEach(p => {
      for (let i = 0; i < p.palabra.length; i++) {
        ...
        const c   = p.direccion === 'H' ? p.col + i : p.col
        const key = getCeldaKey(r, c)
        if (respuestas[key] && respuestas[key] !== p.palabra[i]) {
          nuevosErrores.add(key)
        }
      }
    })
              ...
              <div key={r} className="flex">
                {Array.from({ length: colMax - colMin + 1 }, (_, ci) => {
                  ...
                  return (
                    ...
                    >
                      {num && (
                        <span className="absolute top-0 left-0.5 text-[7px] font-bold text-gray-500 leading-none">{num}</span>
                      )}
